...
function createParametricShapeMesh(shape, size) {
  const type = shape?.generation?.base?.type ?? shape?.shape_family;
  if (!size?.dimensions || !['point_1', 'point_2', 'point_3'].includes(type)) return null;
  const geometry = buildShapeGeometry({ shape, size, scale: CELL_SCALE, symmetry: {}, showVoxels: false });
  const group = meshWithEdges(geometry, createOperationMaterial(), new THREE.LineBasicMaterial({ color: 0x121212, transparent: true, opacity: 0.85 }));
  group.userData.parametricType = type;
  return group;
}

function getParametricFootprint(dim, type) {
  const L = Number(dim.length) || 1;
  const W = Number(dim.width) || 1;
  const H = Number(dim.height) || 1;
  if (type === 'point_1') {
    const shoulder = Math.max(L * 0.68, L - 1.6);
    const noseHalf = Math.min(0.18, W * 0.08);
    return [
      { x: 0, y: W / 2 - noseHalf, zTop: H * 0.18 },
      { x: 0, y: W / 2 + noseHalf, zTop: H * 0.18 },
      { x: shoulder, y: W, zTop: H * 0.82 },
      { x: L, y: W, zTop: H },
      { x: L, y: 0, zTop: H },
      { x: shoulder, y: 0, zTop: H * 0.82 },
    ];
  }
  if (type === 'point_2') {
    const noseHalf = Math.min(0.22, W * 0.1);
    return [
      { x: 0, y: W / 2 - noseHalf, zTop: H },
      { x: 0, y: W / 2 + noseHalf, zTop: H },
      { x: L, y: W, zTop: H },
      { x: L, y: 0, zTop: H },
    ];
  }
  // point_3: right-triangle asymmetric external silhouette. Symmetry buttons produce the other orientations.
  return [
    { x: 0, y: 0, zTop: H },
    { x: L, y: W, zTop: H },
    { x: L, y: 0, zTop: H },
...
function getParametricPrismMesh(dim, type, scale) {
  const footprint = getParametricFootprint(dim, type);
  const vertices = [];
  for (const p of footprint) vertices.push([worldX(p.x, dim), worldY(p.y, dim), worldZ(0, dim)]);
  for (const p of footprint) vertices.push([worldX(p.x, dim), worldY(p.y, dim), worldZ(p.zTop, dim)]);
  ...
  // worldX/worldY/worldZ already apply CELL_SCALE. The scale argument exists for parity with assembly builders.
  void scale;
  return { vertices, faces };
}

function renderOperationSolids(shape, size) {
  const ops = shape.generation?.operations ?? [];
  for (const op of ops) {
    const object = createOperationSolidMesh(op, size);
    if (!object) continue;
    root.add(object);
  }
}

function createOperationSolidMesh(op, size) {
  if (op.type === 'round') {
    if (op.scope?.kind === 'corner') return createCornerRoundMesh(op, size);
    if (['edge_line', 'top_side_line', 'side_line'].includes(op.scope?.kind)) return createRoundRailMesh(op, size);
  }
  if (op.type === 'chamfer') {
    if (op.scope?.kind === 'corner') return createCornerChamferMesh(op, size);
    if (['edge_line', 'top_side_line', 'side_line'].includes(op.scope?.kind)) return createChamferRailMesh(op, size);
  }
  if (op.type === 'slope') return createSlopePreviewMesh(op, size);
  return null;
}
...
function createCornerRoundMesh(op, size) {
  const d = size.dimensions;
  const sides = getCornerSides(op, d);
  const path = roundedCornerPath(sides.x, sides.y, d, 1, 18);
  const shape2d = pathToShape(path);
  const geom = new THREE.ExtrudeGeometry(shape2d, {
    depth: d.height * CELL_SCALE,
    bevelEnabled: false,
    curveSegments: 18,
    steps: 1,
  });
  geom.translate(0, 0, -d.height * CELL_SCALE / 2);
  return meshWithEdges(geom, createOperationMaterial(), createOperationEdgeMaterial(op));
}

function createCornerChamferMesh(op, size) {
  const d = size.dimensions;
  const sides = getCornerSides(op, d);
  const path = chamferCornerPath(sides.x, sides.y, d, 0.5);
  const shape2d = pathToShape(path);
  const geom = new THREE.ExtrudeGeometry(shape2d, { depth: d.height * CELL_SCALE, bevelEnabled: false, steps: 1 });
  geom.translate(0, 0, -d.height * CELL_SCALE / 2);
  return meshWithEdges(geom, createOperationMaterial(), createOperationEdgeMaterial(op));
}

function pathToShape(points) {
  const shape2d = new THREE.Shape();
  points.forEach((p, index) => {
    if (p.arc) {
      shape2d.absarc(
        worldX(p.arc.cx, p.arc.d),
        worldY(p.arc.cy, p.arc.d),
        p.arc.radius * CELL_SCALE,
        p.arc.start,
        p.arc.end,
    ...
    if (index === 0) shape2d.moveTo(worldX(p.x, p.d), worldY(p.y, p.d));
    else shape2d.lineTo(worldX(p.x, p.d), worldY(p.y, p.d));
  });
  shape2d.closePath();
  return shape2d;
}
...
function roundedCornerPath(xSide, ySide, d, radius = 1) {
  const L = d.length;
  const W = d.width;
  if (xSide === 'left' && ySide === 'front') {
    return [
      pt(1, 0, d), pt(1, 1, d), pt(0, 1, d),
      arc(1, 1, radius, Math.PI, Math.PI * 1.5, false, d),
    ];
  }
  if (xSide === 'right' && ySide === 'front') {
    return [
      pt(L - 1, 0, d), pt(L - 1, 1, d), pt(L, 1, d),
      arc(L - 1, 1, radius, 0, -Math.PI / 2, true, d),
    ];
  }
  if (xSide === 'right' && ySide === 'back') {
    return [
      pt(L - 1, W, d), pt(L - 1, W - 1, d), pt(L, W - 1, d),
      arc(L - 1, W - 1, radius, 0, Math.PI / 2, false, d),
    ];
  }
  return [
    pt(1, W, d), pt(1, W - 1, d), pt(0, W - 1, d),
    arc(1, W - 1, radius, Math.PI, Math.PI / 2, true, d),
  ];
}
...
function chamferCornerPath(xSide, ySide, d, amount = 0.5) {
  ...
  const W = d.width;
  if (xSide === 'left' && ySide === 'front') {
    return [pt(1, 0, d), pt(1, 1, d), pt(0, 1, d), pt(0, amount, d), pt(amount, 0, d)];
  }
  if (xSide === 'right' && ySide === 'front') {
    return [pt(L - 1, 0, d), pt(L - 1, 1, d), pt(L, 1, d), pt(L, amount, d), pt(L - amount, 0, d)];
  }
  if (xSide === 'right' && ySide === 'back') {
    return [pt(L - 1, W, d), pt(L - 1, W - 1, d), pt(L, W - 1, d), pt(L, W - amount, d), pt(L - amount, W, d)];
  }
  return [pt(1, W, d), pt(1, W - 1, d), pt(0, W - 1, d), pt(0, W - amount, d), pt(amount, W, d)];
}

function pt(x, y, d) { return { x, y, d }; }
function arc(cx, cy, radius, start, end, clockwise, d) { return { arc: { cx, cy, radius, start, end, clockwise, d } }; }
...
function getCornerSides(op, d) {
  ...
  const cell = op.selection?.cell ?? {};
  x ??= Number(cell.x) === d.length - 1 ? 'right' : 'left';
  y ??= Number(cell.y) === d.width - 1 ? 'back' : 'front';
  return { x, y };
}

function createRoundRailMesh(op, size) {
  return createEdgeProfileMesh(op, size, 'round');
}

function createChamferRailMesh(op, size) {
  return createEdgeProfileMesh(op, size, 'chamfer');
}

function createEdgeProfileMesh(op, size, mode) {
  const d = size.dimensions;
  ...
  if (axis === 'length') {
    const y0 = side === 'front' ? 0 : d.width - 1;
    const y1 = side === 'front' ? 1 : d.width;
    const z0 = face === 'bottom' ? 0 : Math.max(0, d.height - 1);
    const z1 = face === 'bottom' ? Math.min(1, d.height) : d.height;
    const yz = mode === 'round'
      ? roundedSideProfile(side, face, y0, y1, z0, z1, amount)
      : chamferSideProfile(side, face, y0, y1, z0, z1, amount);
    const geom = extrudeProfileAlongX(yz, 0, d.length, d);
    return meshWithEdges(geom, createOperationMaterial(), createOperationEdgeMaterial(op));
  }

  const x0 = side === 'left' ? 0 : d.length - 1;
  const x1 = side === 'left' ? 1 : d.length;
  const z0 = face === 'bottom' ? 0 : Math.max(0, d.height - 1);
  const z1 = face === 'bottom' ? Math.min(1, d.height) : d.height;
  const xz = mode === 'round'
    ? roundedSideProfile(side, face, x0, x1, z0, z1, amount)
    : chamferSideProfile(side, face, x0, x1, z0, z1, amount);
  const geom = extrudeProfileAlongY(xz, 0, d.width, d);
  return meshWithEdges(geom, createOperationMaterial(), createOperationEdgeMaterial(op));
}
...
function roundedSideProfile(side, face, a0, a1, z0, z1, radius = 1) {
  ...
  const points = [];
  if (top && outerIsMin) {
    points.push({ a: innerA, z: z1 }, { a: innerA, z: z0 }, { a: outerA, z: z0 }, { arc: { a: centerA, z: centerZ, radius, start: Math.PI, end: Math.PI / 2, clockwise: true } });
  } else if (top && !outerIsMin) {
    points.push({ a: innerA, z: z1 }, { a: innerA, z: z0 }, { a: outerA, z: z0 }, { arc: { a: centerA, z: centerZ, radius, start: 0, end: Math.PI / 2, clockwise: false } });
  } else if (!top && outerIsMin) {
    points.push({ a: innerA, z: z0 }, { a: innerA, z: z1 }, { a: outerA, z: z1 }, { arc: { a: centerA, z: centerZ, radius, start: Math.PI, end: Math.PI * 1.5, clockwise: false } });
  } else {
    points.push({ a: innerA, z: z0 }, { a: innerA, z: z1 }, { a: outerA, z: z1 }, { arc: { a: centerA, z: centerZ, radius, start: 0, end: -Math.PI / 2, clockwise: true } });
  }
  return points;
}
...
function extrudeProfileAlongX(profile, xMin, xMax, d) {
  const positions = [];
  const indices = [];
  const toWorld = (x, a, z) => [worldX(x, d), worldY(a, d), worldZ(z, d)];
  for (const x of [xMin, xMax]) {
    for (const p of sampleProfile(profile)) positions.push(...toWorld(x, p.a, p.z));
  }
  buildPrismIndices(indices, sampleProfile(profile).length);
...
function extrudeProfileAlongY(profile, yMin, yMax, d) {
  const positions = [];
  const indices = [];
  const sampled = sampleProfile(profile);
  const toWorld = (a, y, z) => [worldX(a, d), worldY(y, d), worldZ(z, d)];
  for (const y of [yMin, yMax]) {
    for (const p of sampled) positions.push(...toWorld(p.a, y, p.z));
  }
...
function sampleProfile(profile) {
  ...
  for (const p of profile) {
    ...
    for (let i = 1; i <= steps; i += 1) {
      ...
      sampled.push({ a: a + Math.cos(t) * radius, z: z + Math.sin(t) * radius });
    }
  }
  return sampled;
}
...
function buildPrismIndices(indices, n) {
  ...
  // cap 1
  const offset = n;
  for (let i = 1; i < n - 1; i += 1) indices.push(offset, offset + i, offset + i + 1);
  // sides
  for (let i = 0; i < n; i += 1) {
    const j = (i + 1) % n;
    indices.push(i, j, offset + j);
    indices.push(i, offset + j, offset + i);
  }
}

function createSlopePreviewMesh(op, size) {
  const d = size.dimensions;
  const geom = new THREE.ConeGeometry(Math.max(d.length, d.width) * CELL_SCALE * 0.35, d.height * CELL_SCALE, 4);
  geom.rotateZ(Math.PI / 4);
  return new THREE.Mesh(geom, createOperationMaterial());
}

function worldX(x, d) { return (x - d.length / 2) * CELL_SCALE; }
function worldY(y, d) { return (y - d.width / 2) * CELL_SCALE; }
function worldZ(z, d) { return (z - d.height / 2) * CELL_SCALE; }
