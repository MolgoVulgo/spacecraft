...
function normalizePoint(point) {
  return {
    id: String(point?.id ?? ''),
    x: roundCutValue(point?.x),
    y: roundCutValue(point?.y),
    z: roundCutValue(point?.z),
  };
}
...
function normalize(vector) {
  ...
  return {
    x: roundCutValue(vector.x / length),
    y: roundCutValue(vector.y / length),
    z: roundCutValue(vector.z / length),
  };
}
...
export function getCutOperations(operations = []) {
  return operations.filter((operation) => operation?.type === 'cut');
}

export function createCutOperation({ shapeId, face, pointMap = new Map(), keepSide = 'normal' }) {
  ...
  return {
    created: true,
    operation: {
      id: `cut_${face.points.join('__')}__${keepSide}`,
      type: 'cut',
      target: 'cut',
      selection: {
        face: 'custom',
      ...
      normal: {
        x: roundCutValue(normal.x),
        y: roundCutValue(normal.y),
        z: roundCutValue(normal.z),
      },
      keep_side: keepSide === 'inverse' ? 'inverse' : 'normal',
      status: 'draft',
      metadata: {
        source: 'editor_advanced_cut',
...
export function validateCutOperation(operation, size = null) {
  const errors = [];
  const warnings = [];
  const points = Array.isArray(operation?.points) ? operation.points : [];

  if (operation?.type !== 'cut') errors.push('type invalide');
  if (points.length < 3) errors.push('au moins 3 points requis');
  if (!['normal', 'inverse'].includes(operation?.keep_side)) errors.push('keep_side invalide');
  ...
  if (normalizedPoints.length >= 3 && !computeNormal(normalizedPoints)) errors.push('plan de coupe dégénéré');
